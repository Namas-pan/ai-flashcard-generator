...
import { buildPrompt, buildUrlPrompt, parseAIResponse } from '../utils/promptBuilder';
...
export interface IAIService {
    ...
    generateFlashcardsFromUrl(url: string, cardTypes: CardType[]): Promise<AIResponse>;
}
...
export class OpenAIProvider implements IAIService {
    ...
    async generateFlashcardsFromUrl(url: string, cardTypes: CardType[]): Promise<AIResponse> {
        const prompt = buildUrlPrompt(url, cardTypes, this.settings.maxCards);
        ...
        try {
            ...
            if (cards.length === 0) {
                throw new Error('无法从 AI 响应中解析出有效的卡片，可能模型不支持读取链接内容');
            }
